...
def create_engine(user, password, database, host='127.0.0.1', port = 3306):
    ...
    import mysql.connector
    if engine is not None:
        raise DBError("Db is not None")
    params = dict(user = user, password = password, database = database, host = host, port = port)
    #connector = mysql.connector.connect(**params)
    engine = _Engine(mysql.connector.connect(**params))
    print 'connet database success'
...
if __name__=='__main__':
    ...
    create_engine(user = 'root', password = 'admin', database='test', host='127.0.0.1', port=3306)
    user = select('select * from user where id = %s', ('1',))
    print user